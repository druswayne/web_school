...
**С17.** Линейная функция $y=kx+b$ имеет угловой коэффициент $k=-2$ и проходит через точку $A(-2;5)$. Найдите формулу функции. Определите координаты точки пересечения её графика с осью абсцисс и промежуток значений аргумента, при которых функция принимает отрицательные значения.
